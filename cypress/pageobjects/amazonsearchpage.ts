...
const brandphone ='li[aria-label="HTC"]'
    ...
    //Search for One Plus Phones
     ...
     clickonphonedetails(text:string){
        // cy.get('.a-offscreen').contains('598').should('be.visible')
         cy.get(oneplusphoneslink).contains(text).click();
         cy.wait(5000)
    ...
    navigatetoonephone(){
        cy.get(oneplusphoneslink).contains('OnePlus-Interstellar-Unlocked-Android-Smartphone').click();
        cy.get(addtocarticon).should('be.visible')
...
//Verify your cart is empty
    emptycart(){
        ...
        cy.wait(2000)
         //cy.get('.div[class="a-row sc-your-amazon-cart-is-empty"]').next('h2').should('contain','Your Amazon Cart is empty');
   ...
   //Click on Add to Cart
    ...
    Selectbrandfilter(text:string){
     let filtervalue = text.split(';');
     for(var i=0;i<filtervalue.length;i++){
         cy.get('li[aria-label="'+filtervalue[i]+ '"]').click({ force: true });
     }


        cy.get('span[class="a-color-base a-text-bold"]').contains(text).should('be.visible')
